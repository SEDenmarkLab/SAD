...
with open('3_1_2023_SAD_774_reactants_1000_entries.pkl', 'rb') as f:
...
for mol_object in remain_mol_0:
    ...
    if np.count_nonzero(mol_bool) == 2:
        ...
        #Sets property "_Alkene" to be equal to numpy boolean array
        mol.mol.SetProp("_Alkene", "".join('1' if v else '0' for v in mol_bool))
        original_array = np.array([True if v == '1' else False for v in mol.mol.GetProp("_Alkene")])
...
with open(f'SAD_Alkene_p0_react_{dt}.pkl', 'wb') as f:
    pickle.dump(final_mol_object_list, f)
print(f'Saved final mol object list as "SAD_Alkene_p0_react_{dt}.pkl"')

if len(not_connected_alkenes) != 0:

    with open(f'SAD_Alkene_p0_react_not_connected_{dt}.pkl', 'wb') as f:
        pickle.dump(not_connected_alkenes, f)
    print(f'Saved not connected alkenes as "SAD_Alkene_p0_react_not_connected_{dt}.pkl"')

if len(multiple_alkenes) != 0:

    with open(f'SAD_Alkene_p0_react_multiple_alkenes_{dt}.pkl', 'wb') as f:
        pickle.dump(multiple_alkenes, f)
    print(f'Saved not connected alkenes as "SAD_Alkene_p0_react_multiple_alkenes_{dt}.pkl"')

if len(problematic_mol_object_list) != 0:

    with open(f'SAD_Alkene_p0_react_problematic_alkenes_{dt}.pkl', 'wb') as f:
        pickle.dump(problematic_mol_object_list, f)
    print(f'Saved problematic alkenes as "SAD_Alkene_p0_react_problematic_alkenes_{dt}.pkl"')
