...
for react_mol in mult_alk:
    ...
    mol_bool[c1] = True
    mol_bool[c2] = True
    react_mol.SetProp("_Alkene_w_H", "".join('1' if v else '0' for v in mol_bool))
    original_array = np.array([True if v == '1' else False for v in react_mol.GetProp("_Alkene_w_H")])
    ...
    if all(original_array == mol_bool):
        ...
        problematic_alkenes.append(react_mol)
    assert react_mol.HasProp("_Alkene_w_H"), f"react_mol doesn't have property"

with open('SAD_Step_3_First_Alkene_Filter_03_01_2023.pkl', 'rb') as f:
...
with open('SAD_Step_3_All_Alkene_Filter_3_01_2023.pkl', 'wb') as f:
