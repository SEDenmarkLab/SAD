...
all_react_desc_csv = glob('*Desc_DF.csv')
db_df = pd.read_csv('p8_reduced_database_column_update_1001.csv')
for csv in all_react_desc_csv:
    react_desc_df = pd.read_csv(csv, index_col=0)
    alkene_type = '_'.join(csv.split('_')[2:4])
    ...
    print(final_desc_df)
    final_desc_df.to_csv(f'Step_8_Altered_Temp_Scaled_ddG_{alkene_type}_Desc_Matrix_Avg.csv')
